...
- Renderers share an interface: `resize / setCanyon / render(state) / project(worldPos) / dispose`. Sparks/bursts are a 2D canvas overlay (`src/particles.js`). The mote's light trail (0.5.0) is volumetric: main.js keeps a 16-point flight history (`trail`, 1-unit spacing), passed per frame as `uTrail` vec4 array (xyz + fade) into the raymarcher; the Three.js fallback draws it as an additive polyline. "stardust" localStorage key still controls it.
